...
export const TieredContextPlugin: Plugin = async ({ directory }) => {
  ...
  return {
    tool: {
      ...
      // ─── semantic_search: TF-IDF style retrieval ─────
      semantic_search: tool({
        description: "Searches memory entries using word-overlap scoring (TF-IDF style). Returns entries ranked by relevance to the query. Use when keyword search in memory_search isn't finding what you need.",
        args: {
          query: tool.schema.string().describe("Natural language query to match against memory"),
          limit: tool.schema.number().describe("Max results").optional().default(5),
        },
        async execute(args) {
          const file = `${MEM_DIR}/store.jsonl`
          if (!existsSync(file)) return { output: "No memory entries found." }
          const raw = readFileSync(file, "utf8").trim().split("\n").filter(Boolean)
          const entries = raw.map(l => { try { return JSON.parse(l) } catch { return null } }).filter(Boolean)

          // Simple TF-IDF style scoring: word overlap with inverse-document-frequency weighting
          const queryWords = (args.query || "").toLowerCase().split(/\W+/).filter(Boolean)
          if (queryWords.length === 0) return { output: "Empty query." }
          ...
          const scored = entries.map(entry => {
            ...
            for (const word of queryWords) {
              if (text.includes(word)) {
                const idf = Math.log((docCount + 1) / (1 + (df[word] || 1)))
                const tf = (text.match(new RegExp(word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), "g")) || []).length
                score += tf * idf
              }
            }
            return { entry, score }
          ...
          const sorted = scored.sort((a, b) => b.score - a.score).filter(s => s.score > 0).slice(0, args.limit || 5)
          if (sorted.length === 0) return { output: `No semantically matching entries for: "${args.query}"` }

          let out = `═══ SEMANTIC SEARCH: "${args.query}" ═══\nFound: ${sorted.length} matches\n\n`
          for (const { entry, score } of sorted) {
            out += `[score: ${score.toFixed(2)}] [${entry.timestamp?.slice(0,10) || "?"}] ${entry.type || "event"}: ${(entry.content || entry.text || "").slice(0, 200)}\n`
            out += `  ID: ${entry.id}\n\n`
          }
          return { output: out.trim() }
        }
